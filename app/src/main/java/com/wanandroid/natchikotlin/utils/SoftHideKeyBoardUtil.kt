...
//        }
        val mSoftHideKeyBoardUtil by lazy<SoftHideKeyBoardUtil> {
    ...
    public fun hideKeyBoardUtil(activity: Activity) { //1､找到Activity的最外层布局控件，它其实是一个DecorView,它所用的控件就是FrameLayout
        val content = activity.findViewById<View>(android.R.id.content) as FrameLayout //2､获取到setContentView放进去的View
        ...
        mChildOfContent?.viewTreeObserver?.addOnGlobalLayoutListener {
            if (isfirst) { //4､Activity中xml布局的高度-当前可用高度
    ...
    private fun possiblyResizeChildOfContent() { //1､获取当前界面可用高度，键盘弹起后，当前界面可用布局会减少键盘的高度
        ...
        if (usableHeightNow != usableHeightPrevious) {  //3､获取Activity中xml中布局在当前界面显示的高度
            val usableHeightSansKeyboard = mChildOfContent?.getRootView()?.getHeight()  //4､Activity中xml布局的高度-当前可用高度
            val heightDifference = usableHeightSansKeyboard?.minus(usableHeightNow)  //5､高度差大于屏幕1/4时，说明键盘弹出
