...
public class PickerView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        drawText(canvas, mLightColor, mScrollDistance, mDataList.get(mSelectedIndex),true);

        // 绘制选中上方的 text
        for (int i = 1; i <= mSelectedIndex; i++) {
            drawText(canvas, mDarkColor, mScrollDistance - i * mTextSpacing,
                    mDataList.get(mSelectedIndex - i),false);
        }
    ...
    private void drawText(Canvas canvas, int textColor, float offsetY, String text,boolean isDrawCenter) {
        ...
        if (isDrawCenter) {
            canvas.drawLine(0f,baseline, mHalfWidth*2,baseline,mPaint);
        }
